...
if (apply) {
    const databaseCount = await database().query<{ count: string }>(
        'SELECT count(*)::text AS count FROM legacy_imports WHERE source_path LIKE $1',
        [`progress/%/labs/${LAB_ID}`],
    );
    console.log(
        JSON.stringify(
            {
                ...counts,
                databaseCount: Number(databaseCount.rows[0].count),
                verified: counts.unsupported === 0 && Number(databaseCount.rows[0].count) === counts.source,
            },
            null,
            2,
        ),
