...
import { ConflictError, createScenario, getScenario, listRevisions, listScenarios, NotFoundError, updateScenario } from '../store.js';

if (process.env.RUN_DB_TESTS === 'true' && !process.env.TEST_DATABASE_URL) {
  try { process.loadEnvFile('.env.vercel.local'); } catch { /* The caller may supply TEST_DATABASE_URL directly. */ }
  process.env.TEST_DATABASE_URL = process.env.DATABASE_URL;
}

test('PostgreSQL isolates owners and preserves immutable scenario revisions', { skip: !process.env.TEST_DATABASE_URL }, async () => {
  process.env.DATABASE_URL = process.env.TEST_DATABASE_URL;
  const owner = `test-${randomUUID()}`;
  const stranger = `test-${randomUUID()}`;
  const input = { title: 'First architecture', region: 'us-east-1', workloadAssumptions: { requestsPerMonth: 1000 }, configuration: { resources: [] } };
  try {
    ...
    const updated = await updateScenario(owner, created.id, { ...input, title: 'Revised architecture' }, 1);
    ...
    assert.deepEqual((await listRevisions(owner, created.id)).map((item) => item.revision), [2, 1]);
  } finally {
