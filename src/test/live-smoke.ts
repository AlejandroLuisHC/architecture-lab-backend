...
for (const file of ['.env', '.env.vercel.local', '../architecture-lab-frontend/.env.local']) {
  try { process.loadEnvFile(file); } catch { /* Explicit environment variables also work. */ }
}

const apiKey = process.env.VITE_FIREBASE_API_KEY;
const baseUrl = process.env.SMOKE_API_URL;
if (!apiKey || !baseUrl || !process.env.DATABASE_URL) throw new Error('Firebase API key, SMOKE_API_URL, and DATABASE_URL are required.');
...
try {
  ...
  const saved = await call('/me/progress/serverless-web', ownerToken, 'PUT', { version: LAB_VERSION, configuration: { resources: [] }, currentStep: 0 });
  ...
  const created = await call('/me/scenarios', ownerToken, 'POST', { title: 'Smoke test', region: 'us-east-1', configuration: { resources: [] } });
  ...
  const changed = await call(`/me/scenarios/${scenarioId}`, ownerToken, 'PUT', { title: 'Smoke test revised', region: 'us-east-1', configuration: { resources: [] }, expectedRevision: 1 });
  ...
  assert.equal((await call(`/me/scenarios/${scenarioId}`, ownerToken, 'PUT', { title: 'Stale', region: 'us-east-1', configuration: { resources: [] }, expectedRevision: 1 })).status, 409);

  const persisted = await database().query<{ count: string }>('SELECT count(*)::text AS count FROM module_attempts a JOIN scenarios s ON s.id=a.scenario_id JOIN workspaces w ON w.id=s.workspace_id WHERE w.owner_uid=$1', [ownerUid]);
  assert.equal(Number(persisted.rows[0].count), 1);
  console.log('Live smoke check passed: Firebase token, PostgreSQL progress, scenario ownership, revisions, and conflict response.');
} finally {
  if (users.length) {
