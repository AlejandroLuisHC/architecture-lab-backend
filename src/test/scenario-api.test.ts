...
test(
    'scenario API enforces ownership and optimistic revision checks',
    ...
    async () => {
        ...
        try {
            ...
            const created = (await createdResponse.json()).scenario as {
                id: string;
            ...
            const strangerRead = await fetch(`${baseUrl}/me/scenarios/${created.id}`, {
            ...
            const ownerUpdate = await fetch(`${baseUrl}/me/scenarios/${created.id}`, {
                method: 'PUT',
                headers: auth(owner),
                body: JSON.stringify({ ...input, title: 'Revised architecture', expectedRevision: 1 }),
            });
            ...
            const revisionsResponse = await fetch(`${baseUrl}/me/scenarios/${created.id}/revisions`, {
                headers: auth(owner),
            });
            assert.equal(revisionsResponse.status, 200);
            assert.deepEqual(
                (await revisionsResponse.json()).revisions.map(
                    (revision: { revision: number }) => revision.revision,
                ),
                [2, 1],
            );

            const strangerRevisions = await fetch(`${baseUrl}/me/scenarios/${created.id}/revisions`, {
                headers: auth(stranger),
            });
