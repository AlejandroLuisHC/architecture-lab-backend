...
async function publishModule(client: PoolClient) {
    ...
    const result = await client.query<{ content_hash: string }>(
        'SELECT content_hash FROM module_versions WHERE module_id=$1 AND version=$2',
        [LAB_ID, LAB_VERSION],
    );
    if (result.rows[0]?.content_hash !== hash)
        throw new ConflictError('The published lab definition changed without a version increase.');
}
...
async function personalWorkspace(client: PoolClient, uid: string): Promise<string> {
    ...
    await client.query(
        'INSERT INTO workspaces(id, owner_uid) VALUES($1,$2) ON CONFLICT(owner_uid) DO NOTHING',
        [randomUUID(), uid],
    );
...
async function writeResources(client: PoolClient, scenarioId: string, input: ScenarioInput) {
    ...
    for (const resource of input.configuration.resources) {
        await client.query(
            'INSERT INTO scenario_resources(scenario_id,id,service,resource_type,schema_version,name,region,config) VALUES($1,$2,$3,$4,$5,$6,$7,$8::jsonb)',
            [
                scenarioId,
                resource.id,
                serviceFor(resource.type, resource),
                resource.type,
                resource.type === 'awsResource' ? resource.schemaVersion : 1,
                resource.name,
                input.region,
    ...
    for (const link of relationships) {
        await client.query(
            'INSERT INTO scenario_relationships(scenario_id,source_id,target_id,kind) VALUES($1,$2,$3,$4)',
            [scenarioId, link.sourceId, link.targetId, link.kind],
        );
    }
}
...
type CreateScenarioOptions = {
    ...
    origin?: 'created-in-sandbox' | 'imported';
    originMetadata?: object;
};

async function createScenarioTx(options: CreateScenarioOptions): Promise<string> {
    const { client, workspaceId, input, mode } = options;
    const origin = options.origin ?? 'created-in-sandbox';
    ...
    await client.query(
        'INSERT INTO scenario_revisions(scenario_id,revision,snapshot) VALUES($1,1,$2::jsonb)',
        [id, JSON.stringify(snapshot(input))],
    );
    return id;
}

async function ownedScenario(
    client: PoolClient,
    uid: string,
    id: string,
    lock = false,
...
async function updateScenarioTx(client: PoolClient, row: ScenarioRow, input: ScenarioInput): Promise<number> {
    ...
    await client.query(
        ...
        [row.id, revision, JSON.stringify(snapshot(input))],
    );
    return revision;
}
...
function calculatedProgress(
    input: ProgressInput,
    previous?: AttemptRow,
    importedCompletedAt?: string | null,
): StoredProgress {
    ...
    return {
        ...
        completedAt: evaluation.complete
            ? (importedCompletedAt ?? previous?.completed_at?.toISOString() ?? new Date().toISOString())
            : null,
    };
}
...
export async function getProgress(uid: string): Promise<StoredProgress | null> {
    return transaction(async (client) => {
        ...
        if (attempt.module_version !== LAB_VERSION)
            throw new LabVersionMismatchError(
                'This saved run belongs to an older lab version. Start a new run to continue.',
            );
        const revision = await client.query<{ snapshot: { configuration: LabConfiguration } }>(
            'SELECT snapshot FROM scenario_revisions WHERE scenario_id=$1 AND revision=(SELECT current_revision FROM scenarios WHERE id=$1)',
            [attempt.scenario_id],
        );
        return {
            version: attempt.module_version,
            configuration: revision.rows[0].snapshot.configuration,
            currentStep: attempt.current_step,
            unlockedThroughStep: attempt.unlocked_through_step,
...
export async function saveProgress(uid: string, input: ProgressInput): Promise<StoredProgress> {
    return transaction(async (client) => {
        ...
        if (current) {
            ...
            await client.query(
                'UPDATE module_attempts SET current_step=$2,unlocked_through_step=$3,completed_at=$4,updated_at=$5 WHERE id=$1',
                [
                    current.id,
                    progress.currentStep,
                    progress.unlockedThroughStep,
                    progress.completedAt,
                    progress.updatedAt,
                ],
            );
        } else {
            const scenarioId = await createScenarioTx({
                client,
                workspaceId,
                input: scenarioInput,
                mode: 'guided',
            ...
            await client.query(
                'INSERT INTO module_attempts(id,scenario_id,module_id,module_version,current_step,unlocked_through_step,completed_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',
                [
                    randomUUID(),
                    scenarioId,
                    LAB_ID,
...
export async function importLegacyProgress(
    uid: string,
    sourcePath: string,
    legacy: StoredProgress,
): Promise<'imported' | 'already-imported' | 'unsupported'> {
    if (!legacyProgressSchema.safeParse(legacy).success || legacy.version !== LAB_VERSION)
        return 'unsupported';
    const parsed = scenarioInputForLegacy(legacy);
    if (!parsed) return 'unsupported';
    return transaction(async (client) => {
        await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [uid]);
        const existing = await client.query('SELECT 1 FROM legacy_imports WHERE source_path=$1', [
            sourcePath,
        ]);
        ...
        const scenarioId = await createScenarioTx({
            ...
            mode: 'guided',
            origin: 'imported',
            originMetadata: { source: 'firestore-progress', sourcePath },
        });
        ...
        await client.query(
            ...
            [
                ...
                LAB_ID,
                LAB_VERSION,
                Math.min(legacy.currentStep, progress.unlockedThroughStep),
                Math.max(legacy.unlockedThroughStep, progress.unlockedThroughStep),
                progress.completedAt,
                legacy.updatedAt,
            ],
        );
        await client.query('INSERT INTO legacy_imports(source_path,scenario_id) VALUES($1,$2)', [
            sourcePath,
            scenarioId,
        ]);
        return 'imported';
...
export async function listScenarios(uid: string) {
    return transaction(async (client) => {
        ...
        const result = await client.query<
            ScenarioRow & { module_id: string | null; module_version: number | null }
        >(
            'SELECT s.*,a.module_id,a.module_version FROM scenarios s LEFT JOIN module_attempts a ON a.scenario_id=s.id WHERE s.workspace_id=$1 ORDER BY s.updated_at DESC',
            [workspaceId],
        );
...
export async function createScenario(uid: string, input: ScenarioInput) {
    return transaction(async (client) => {
        const workspaceId = await personalWorkspace(client, uid);
        const id = await createScenarioTx({ client, workspaceId, input, mode: 'freeform' });
...
export async function getScenario(uid: string, id: string, revision?: number) {
    return transaction(async (client) => {
        ...
        const found = await client.query<{ snapshot: unknown; created_at: Date }>(
            'SELECT snapshot,created_at FROM scenario_revisions WHERE scenario_id=$1 AND revision=$2',
            [id, selectedRevision],
        );
        if (!found.rows[0]) throw new NotFoundError('Revision not found.');
        const attempt = await client.query<AttemptRow & { module_id: string }>(
            'SELECT * FROM module_attempts WHERE scenario_id=$1',
            [id],
        );
        return {
            id,
            ...
            workloadAssumptions: row.workload_assumptions,
            currentRevision: row.current_revision,
            revision: selectedRevision,
            snapshot: found.rows[0].snapshot,
            revisionCreatedAt: found.rows[0].created_at.toISOString(),
            attempt: attempt.rows[0]
                ? {
                      moduleId: attempt.rows[0].module_id,
                      moduleVersion: attempt.rows[0].module_version,
                      currentStep: attempt.rows[0].current_step,
                      unlockedThroughStep: attempt.rows[0].unlocked_through_step,
                      completedAt: attempt.rows[0].completed_at?.toISOString() ?? null,
                  }
                : null,
        };
    });
...
export async function listRevisions(uid: string, id: string) {
    return transaction(async (client) => {
        ...
        const result = await client.query<{ revision: number; created_at: Date }>(
            'SELECT revision,created_at FROM scenario_revisions WHERE scenario_id=$1 ORDER BY revision DESC',
            [id],
        );
        return result.rows.map((row) => ({
...
export async function updateScenario(
    ...
    return transaction(async (client) => {
        ...
        if (row.current_revision !== expectedRevision)
            throw new ConflictError('This scenario changed. Reload it before saving.');
        if (row.mode === 'guided')
            throw new ConflictError('Use the module progress route to edit a guided scenario.');
        return { id, currentRevision: await updateScenarioTx(client, row, input) };
    });
}

type RecordAnalysisRunOptions = {
    uid: string;
    scenarioId: string;
    revision: number;
...
export async function recordAnalysisRun(options: RecordAnalysisRunOptions) {
    ...
    return transaction(async (client) => {
        ...
        for (const catalog of catalogs) {
            ...
            await client.query(
                'INSERT INTO source_catalog_versions(id,source_kind,source_url,region,effective_at,fetched_at,coverage,content_hash) VALUES($1,$2,$3,$4,$5,$6,$7::jsonb,$8)',
                [
                    id,
                    'aws-price-list-bulk',
                    `https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/${catalog.serviceCode}/current/${catalog.region}/index.json`,
                    catalog.region,
                    catalog.publicationDate,
                    catalog.fetchedAt,
                    JSON.stringify({ serviceCode: catalog.serviceCode }),
                    contentHash,
                ],
            );
            catalogIds.push(id);
        ...
        const id = randomUUID();
        await client.query(
            'INSERT INTO analysis_runs(id,scenario_id,revision,evaluator_version,source_catalog_ids,assumptions,result) VALUES($1,$2,$3,$4,$5::uuid[],$6::jsonb,$7::jsonb)',
            [
                id,
                scenarioId,
                revision,
